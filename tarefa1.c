...
int main(){
    ...
    DistanciaPar *copiaDistancias = distancias;
    ...
    for (int i = 0; i < num_combinacoes; i++) {
        fprintf(output_csv, "%d,,%f,%d,%f,%f,%f\n", distancias[i].ponto1 + 1,dados[distancias[i].ponto1].X ,distancias[i].ponto2 + 1,dados[distancias[i].ponto2] ,copiaDistancias[i].distancia , distancias[i].distancia);
    }
